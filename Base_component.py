...
class Flynny_image:
    ...
    def open_image(
        img_source_input,
    ):  # image to numpy array, allows for unicode characters in path
    ...
    def scaling(image, percentage):
        ...
        target_size = (
            int(image.shape[1] * percentage),
            int(image.shape[0] * percentage),
        )
        ...
        if a.any() != 0:
            if image.shape[2] >= 4:
                ...
                b, g, r = (
                    ...
                    np.bitwise_and(a, g),
                    np.bitwise_and(a, r),
                )  # combine alpha and color channels, later merge
    ...
    def info_overlay(self, image, info):
        info = "\n".join(
            (
                f"fps: {info['fps']} Frame time (ms): {(info['time']*100):.1f}",
...
def main(args, need_restart, queue):
    ...
    frame_time, info, i, volume_2, volume_3 = time(), 0, 0, 0, 0

    # open img and scale/apply alpha when needed
    img_src = Flynny_image.open_image(args.image)

    if args.scale != 100:
        img_src = Flynny_image.scaling(img_src, args.scale)
    if img_src.shape[2] == 4:
        alpha = Flynny_image.apply_alpha(img_src, retrieve_alpha=True)
    else:
        alpha = np.array([])
    ...
    if args.mask:
        ...
        if args.scale != 100:
            mask = Flynny_image.scaling(mask, args.scale)
        if mask.shape[2] >= 3:
    ...
    with mic.recorder(samplerate=1000, blocksize=16) as stream:
        while cv2.getWindowProperty(args.title, cv2.WND_PROP_VISIBLE):
            ...
            timerStart = time()
            i = (i + 1) % 179
            volume_real = Flynny_audio.audio_sens(stream, args.dampening)

            # smoothing
            if not (i % args.smoothing):
            ...
            if alpha.size == 0:
                ...
                img_out = Flynny_image.apply_alpha(img_HSV2BGR, alpha)

            # stats overlay
            if args.stats and info:
                Flynny_image.info_overlay(Flynny_image, img_out, info)
...
if __name__ == "__main__":
    ...
    def argument_parse(args=sys.argv[1:]):
        ...
        args_main.add_argument(
            ...
            metavar="",
            default=None,
            help="Apply grayscale mask image",
        )
