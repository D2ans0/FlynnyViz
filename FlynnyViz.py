...
def audio_sens(audio_stream):
	...
	data_merged = (np.max(data) - np.min(data))*1000/dampen
	return data_merged
...
def info_overlay(image, x, y, scale, face, color, info):
	...
	cv2.putText(image, micname, (x,y*4), face, scale, color)
...
if __name__ == "__main__":
	...
	cv2.namedWindow(args.title, flags=(cv2.WINDOW_GUI_NORMAL + cv2.WINDOW_AUTOSIZE)) #init main window
	mic = audio_stream()
	micname = str(mic)
	
	#variables setup
	dampen = 2**args.dampening
	...
	try:
		...
		with open('FlynnyViz.log', 'a') as log:
			log.write(f"{repr(err)} {strftime('%d/%m/%Y %H:%M:%S')}\n")
		with mic.recorder(samplerate=1000, blocksize=16) as stream:
			mainloop()